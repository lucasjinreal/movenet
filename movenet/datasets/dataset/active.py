...
class ACTIVE(data.Dataset):
    ...
    mean = np.array([1.0, 1.0, 1.0], dtype=np.float32).reshape(1, 1, 3)
    ...
    def __init__(self, opt, split, sp=False):
        ...
        )  # mli: the dir name is specified by `opt.dataset`
        self.img_dir = os.path.join(self.data_dir, "{}".format(split))
        if split == "test":
            raise ValueError("No supported for the testing dataset.")
        else:
            self.annot_path = os.path.join(
                self.data_dir, "annotations", "{}_{}.json"
            ).format(opt.dataset, split)
        self.max_objs = (
            2  # mli: only consider the images with less than 2 human objects
        )
    ...
    def convert_eval_format(self, all_dets):
        ...
        for image_id in all_dets:
            ...
            keypoints = np.concatenate(
                [dets[:, [1, 0]], np.ones((17, 1), dtype=np.float32)], axis=1
            )
