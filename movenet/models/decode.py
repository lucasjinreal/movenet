...
def _topk_channel_with_reg_kps(scores, reg_kps, K=40, delta=1.0, multipiler=5.0):
    ...
    weight_to_joints = torch.from_numpy(
        multipiler / (np.sqrt(x * x + y * y) + delta)
    ).to(scores.device)
...
def single_pose_decode(heat, wh, kps, reg=None, hm_hp=None, hp_offset=None, K=1):
    """
    ...
    if hm_hp is not None:
        ...
        #     hm_hp, K=K)  # b x J x K
        hm_score, hm_inds, hm_ys, hm_xs = _topk_channel_with_reg_kps(
            hm_hp, reg_kps, K=K
        )  # b x J x K
